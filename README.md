...
**[Перейти к работающей демонстрации Nomad Coders](https://surplus-tex-converter-balance.trycloudflare.com)**

Вход и установка не нужны. Выберите «Расчёт закупки · синтетические наблюдения» и нажмите «Получить рекомендации». Ниже — сценарий проверки за три минуты.

Ссылка проверена 23.09.2026: HTTPS, расчёт, сохранение, согласование и CSV. Это временная публикация через Cloudflare Tunnel: сервер работает на ноутбуке капитана, который должен оставаться включённым и подключённым к интернету. Репозиторий остаётся приватным; исходные данные партнёра не публикуются.
...
Сейчас работает ссылка Cloudflare Tunnel в начале README. Контейнер приложения и PostgreSQL запущены на компьютере капитана; рестарт приложения не удаляет заказы. Доступность зависит от этого компьютера и туннеля.

Для независимого от ноутбука хостинга готовы `Dockerfile` и `render.yaml`: **Render Free + Neon Free**. Render пока требует разрешения организации на приватный репозиторий. Пошаговая настройка и ограничения: [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md).
...
Render Free может засыпать после простоя; Cloudflare Quick Tunnel также не даёт гарантии доступности. Перед защитой проверьте ссылку с другого устройства. Локальная версия остаётся резервом.
